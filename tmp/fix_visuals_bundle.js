...
// --- 1. Fix CompetidoresSeccion.tsx logos array and card titles ---
const file1 = path.join('d:', 'repositorio_geral', 'app_gestao_contratos', 'src', 'components', 'CompetidoresSeccion.tsx');
if (fs.existsSync(file1)) {
    ...
    console.log("Competidores - Start Index: ", startIndex, " End Index: ", endIndex);
    ...
    if (startIndex !== -1 && endIndex !== -1) {
         ...
         console.log("Successfully updated CompetidoresSeccion.");
    } else {
         console.log("Could not update CompetidoresSeccion bounds error.");
    }
}

// --- 2. Fix DiagnosticoHub.tsx remove imageOverlayLight ---
const file2 = path.join('d:', 'repositorio_geral', 'app_gestao_contratos', 'src', 'components', 'DiagnosticoHub.tsx');
if (fs.existsSync(file2)) {
    let content = fs.readFileSync(file2, 'utf-8');
    const lines = content.replace(/\r\n/g, '\n').split('\n');

    for (let i = 0; i < lines.length; i++) {
         if (lines[i].includes("<div className={styles.imageOverlayLight}")) {
              console.log(`Found imageOverlayLight on line ${i+1}, removing...`);
              lines[i] = ""; // Remove the line
              break;
         }
    }
    fs.writeFileSync(file2, lines.join('\n'), 'utf-8');
    console.log("Successfully updated DiagnosticoHub.");
}
