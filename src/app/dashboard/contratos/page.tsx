    ...
    <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ color: 'var(--secondary)', fontSize: '0.9rem', marginTop: '0.2rem' }}>Crie e customize templates de contratos para fornecedores e prestadores.</p>
        </div>
      ...
      {contractTemplates.length === 0 ? (
        ...
        <div style={{ background: '#111318', border: '1px solid #1F242D', borderRadius: '16px', padding: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.45rem', marginBottom: '1.25rem', borderBottom: '1px solid #1F242D', paddingBottom: '0.75rem' }}>
            ...
            <h2 style={{ fontSize: '1rem', fontWeight: 800, color: '#F4F2ED', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
              Modelos de Contratos <span style={{ color: '#8A8F99', fontSize: '0.8rem', textTransform: 'none', fontWeight: 400 }}>({contractTemplates.length})</span>
            </h2>
          </div>
            ...
            {contractTemplates.map((t: any) => (
              <div key={t.id} style={{ background: '#0A0C0F', border: '1px solid #1F242D', borderRadius: '12px', padding: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                  {/* Popular botões */}
                  <Link 
                    href={`/dashboard/contratos/emitir/${t.id}`} 
                    style={{ 
                      color: '#0A0C0F', 
                      background: '#C8F542', 
